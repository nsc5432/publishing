 ...
 */
export function useBaseInfo(ymd: string): Fetched<DsbdBaseInfoDto> {
    // 기준일자만 조건이라 문자열 하나로 충분하다 (빈 값이면 조회하지 않는다).
    const query = useMemo(() => ymd || null, [ymd]);

    return useFetched(
        query,
        (baseYmd) => dashboardService.getBaseInfo(baseYmd).then((dto) => unwrap(dto, FAIL_MESSAGE)),
        FAIL_MESSAGE,
    );
}
