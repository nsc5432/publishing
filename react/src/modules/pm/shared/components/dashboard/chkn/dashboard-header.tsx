...
import { SelectIsland } from '../../select-island';
import type { SimulationType } from '../airport-dashboard';

interface DashboardHeaderProps {
    simulationType: SimulationType;
    onClickSearch: (tmnlId: string, island: string) => void;
}
...
export function DashboardHeader({ simulationType, onClickSearch }: DashboardHeaderProps) {
    ...
    const [searchIsland, setSearchIsland] = useState<string>('A');

    useEffect(() => {
        onClickSearch(searchTmnlId, searchIsland);
    }, []);

    const onChangeTerminal = (value: string) => {
        setSearchTmnlId(value);
        setSearchIsland('A');
    }
                ...
                <div className="flex items-start justify-between gap-6">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2.5 rounded-lg border border-white/20">
                            ...
                            <SelectTmnl tmnlId={searchTmnlId} onChange={onChangeTerminal} />
                        </div>

                        <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2.5 rounded-lg border border-white/20">
                            <span className="text-sm font-medium text-white/90">아일랜드</span>
                            <SelectIsland tmnlId={searchTmnlId} value={searchIsland} onChange={setSearchIsland} />
                        </div>
