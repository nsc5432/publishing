...
--   4) 체크인유형 시트가 읽는 TN_PM_SMLT_CKNCT_TYPE_ATRB 의 실재와 컬럼명
--      SELECT COLUMN_NAME, DATA_TYPE FROM ALL_TAB_COLUMNS
--       WHERE OWNER='PMOWN' AND TABLE_NAME='TN_PM_SMLT_CKNCT_TYPE_ATRB' ORDER BY COLUMN_ID;
--      모바일 비율·값 컬럼은 MOBL_RT/MOBL_VL 이어야 한다.
...
--      SELECT CKNCT_TYPE_ATRB_ID, COUNT(*) FROM PMOWN.TN_PM_SMLT_CKNCT_TYPE_ATRB
...
UPDATE PMOWN.TN_PM_SMLT_PSG_FIX_PARA_CD
 ...
 WHERE PSG_FIX_PARA_GROUP_CD IN (
           ...
           '18010000',  -- 출국심사 내국인
           '18020000',  -- 출국심사 외국인
           -- 발생시각 (EntryTime) T1 P1~P5
           '05010000', '05020000', '05030000', '05040000', '05050000',
           -- 발생시각 (EntryTime) T2 P1~P5
           '05060000', '05070000', '05080000', '05090000', '05100000',
           -- 체크인 리포팅 T1/T2
           '13010000', '13020000',
           '13030000', '13040000',
           -- 게이트 리포팅 (국제선)
           ...
           -- 보안 리포팅 T2 P1~P5
           '15070000', '15080000', '15090000', '15100000', '15110000',
           -- 출국장이용분포 T1 A~Z
...
-- -------------------------------------------------------------------------------------
CREATE TABLE PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY
(
    ...
    CONSTRAINT TH_PM_SMLT_ATRB_APLY_HSTRY_PK PRIMARY KEY (APLY_SN),
    CONSTRAINT TH_PM_SMLT_ATRB_APLY_HSTRY_CK1 CHECK (CNCL_YN IN ('Y', 'N'))
);

COMMENT ON TABLE  PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY                       IS 'PM_시뮬레이션속성적용이력';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.APLY_SN               IS '적용일련번호';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.SRC_FIX_ATRB_GROUP_ID IS '원본고정속성그룹아이디';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.TGT_FIX_ATRB_GROUP_ID IS '대상고정속성그룹아이디';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.TMNL_ID               IS '터미널아이디';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.TBL_NM                IS '테이블명';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.SHEET_NM              IS '시트명';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.APLY_ROW_CNT          IS '적용행수';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.CNCL_YN               IS '취소여부';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.CNCL_DT               IS '취소일시';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.CNCL_RGTR_ID          IS '취소자아이디';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.FRST_RGTR_ID          IS '최초등록자아이디';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.FRST_RGTR_IP_ADDR     IS '최초등록자IP주소';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY.FRST_REG_DT           IS '최초등록일시';

CREATE INDEX PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_IX1
    ON PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY (TGT_FIX_ATRB_GROUP_ID, FRST_REG_DT DESC);

CREATE INDEX PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_IX2
    ON PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY
...
-- -------------------------------------------------------------------------------------
CREATE TABLE PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL
(
    APLY_SN   NUMBER(10)   NOT NULL,
    ...
    BEF_VL    VARCHAR2(500),
    AFT_VL    VARCHAR2(500),
    CONSTRAINT TH_PM_SMLT_ATRB_APLY_HSTRY_DTL_PK
        PRIMARY KEY (APLY_SN, ATRB_CD, DTL_SE_CD, COLUMN_NM),
    CONSTRAINT TH_PM_SMLT_ATRB_APLY_HSTRY_DTL_FK1
        FOREIGN KEY (APLY_SN) REFERENCES PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY (APLY_SN)
);

COMMENT ON TABLE  PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL           IS 'PM_시뮬레이션속성적용이력상세';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL.APLY_SN   IS '적용일련번호';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL.ATRB_CD   IS '속성코드';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL.DTL_SE_CD IS '상세구분코드';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL.COLUMN_NM IS '컬럼명';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL.BEF_VL    IS '변경전값';
COMMENT ON COLUMN PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL.AFT_VL    IS '변경후값';

-- -------------------------------------------------------------------------------------
-- 수동 복구
--   적용 도중 세션이 끊겨 이력만 남고 값이 반쯤 반영된 경우, 상세의 BEF_VL 로 되돌린다.
--     UPDATE PMOWN.TN_PM_SMLT_PSG_ATRB A
--        SET A.INPT_VL = (SELECT D.BEF_VL FROM PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL D
...
--        AND EXISTS (SELECT 1 FROM PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY_DTL D
--                     WHERE D.APLY_SN = &aply_sn AND D.COLUMN_NM = 'INPT_VL'
--                       AND D.ATRB_CD = A.PSG_ATRB_CD AND D.DTL_SE_CD = A.PSG_DTL_SE_CD);
--     UPDATE PMOWN.TH_PM_SMLT_ATRB_APLY_HSTRY SET CNCL_YN = 'Y', CNCL_DT = CURRENT_TIMESTAMP
